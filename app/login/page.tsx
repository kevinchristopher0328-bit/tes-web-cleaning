    ...
    try {
      if (tab === "masuk") {
        ...
        if (data.session) {
          ...
          // Konfirmasi email aktif — belum ada sesi.
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-md px-6 py-12">
        <h1 className="text-2xl font-bold text-foreground">
          {tab === "masuk" ? "Masuk ke Beres" : "Daftar akun Beres"}
        </h1>
        <p className="mt-1 text-sm text-muted-foreground">
          {tab === "masuk"
            ? "Masuk untuk melacak pesanan & menyimpan alamat."
            : "Buat akun untuk pemesanan lebih cepat."}
        </p>

        {/* Tabs */}
        <div className="mt-6 grid grid-cols-2 rounded-xl border border-border bg-muted/50 p-1">
        ...
        <form onSubmit={handleEmail} className="mt-6 space-y-4">
          {tab === "daftar" && (
            <label className="block">
              <span className="mb-1.5 block text-sm font-medium text-foreground">Nama lengkap</span>
              <input
                value={fullName}
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full rounded-xl bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground shadow-sm transition-colors hover:bg-primary-hover disabled:opacity-60"
          >
        ...
        <button
          ...
          className="flex w-full items-center justify-center gap-2 rounded-xl border border-border bg-card px-6 py-3 text-sm font-semibold text-card-foreground transition-colors hover:bg-muted"
        >
          <GoogleIcon />
